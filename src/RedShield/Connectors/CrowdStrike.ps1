...
function Start-CrowdStrike-EvidenceCollection { [CmdletBinding(SupportsShouldProcess)] param([string]$DeviceId) $device = $DeviceId; if ($PSCmdlet.ShouldProcess("$device", "Start evidence collection")) { throw "NotImplemented: Implement triage collection for device $device" } }
